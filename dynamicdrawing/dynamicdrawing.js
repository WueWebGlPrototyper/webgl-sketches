...
var lines = new Float32Array( 200 * 3 );
...
var colorPoints = [ 0.5, 0.5, 1.0, 0.25 ];
...
for( var i = 0; i < lines.length; i += 3 ) {
    out[ 0 ] = lines[ i ] + (Math.random() * 0.02 - 0.01);
    out[ 1 ] = lines[ i + 1] + (Math.random() * 0.02 - 0.01);
    out[ 2 ] = lines[ i + 2] + (Math.random() * 0.02 - 0.01);
    // vec3.normalize( out, out );
    lines[ i ] = out[ 0 ];
	lines[ i + 1 ] = out[ 1 ];
    lines[ i + 2 ] = out[ 2 ];
  ...
  vao.bind()
  vao.draw( gl.TRIANGLES, lines.length / 3.0 );
  vao.unbind();
}
